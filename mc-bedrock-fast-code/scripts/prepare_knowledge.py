...
def main() -> int:
    ...
    runs: list[dict[str, object]] = registry["runs"]  # type: ignore[assignment]
    ...
    if args.remote_indexes:
        ...
        result = run(cmd)
        runs.append({"kind": "remote_public_indexes", **result})
    ...
    if args.api_docs:
        ...
        runs.append({"kind": "api_docs", "out": str(api_out), **result})
        if result["returncode"] != 0:
            write_registry(registry_path, registry)
            return int(result["returncode"])
    ...
    if args.demo_source:
        ...
        if args.pretty:
            cmd.append("--pretty")
        result = run(cmd)
        runs.append({"kind": "demo_index", "out": str(demo_out), "sources": args.demo_source, **result})
        if result["returncode"] != 0:
            write_registry(registry_path, registry)
            return int(result["returncode"])
    ...
    if args.vanilla_index or args.list_versions_only:
        ...
        if args.pretty:
            cmd.append("--pretty")
        result = run(cmd)
        runs.append({"kind": "vanilla_index", "out": str(vanilla_out), "list_only": args.list_versions_only, **result})
        if result["returncode"] != 0:
            write_registry(registry_path, registry)
            return int(result["returncode"])
